...
1. 修改 `content.md`
2. 按 `Cmd+Shift+D`（Mac）或 `Ctrl+Shift+D`（Windows）
3. 等待终端显示部署完成
...
# 带自定义消息
npm run deploy:msg --msg="更新了首页标语"
```

#### 方法 3：VS Code 任务

1. 按 `Cmd+Shift+P` → 输入 "Run Task"
2. 选择 "🚀 部署到生产环境"
3. 等待完成
...
修改 content.md
       ↓
运行 npm run deploy（自动执行）
       ↓
1. 解析 content.md → 生成 src/lib/content.ts
       ↓
2. git add -A
       ↓
3. git commit -m "update: 网站内容更新 02-17 15:30"
       ↓
...
- **content.md** 不会上传到 GitHub（被 .gitignore 排除）
- **src/lib/content.ts** 必须提交（这是实际部署的文件）
- 不要手动编辑 content.ts，它由脚本自动生成
