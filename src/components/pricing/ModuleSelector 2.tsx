...
export function ModuleSelector() {
  ...
  const { selectedModules, addModule, removeModule, setModules: setStoreModules } = usePricingStore()
  ...
  const handlePluginToggle = (module: Module) => {
    ...
    if (isSelected) {
      ...
      addModule({
        ...
        name: module.name,
        type: module.type,
        priceMin: module.price_min || 0,
        priceMax: module.price_max || 0,
        priceUnit: module.price_unit || ''
      })
    }
  }
        ...
        <div className="space-y-4">
          {pluginModules.map((module) => (
            <ModuleCard
              key={module.id}
              module={module}
              selected={!!selectedModules.find(m => m.id === module.id)}
              onSelect={() => handlePluginToggle(module)}
            />
          ))}
        </div>
      </div>
        ...
        >
          {subscriptionModules.map((module) => (
            <Card 
              key={module.id}
              className={`border-2 transition-all cursor-pointer ${
                selectedSubscription?.id === module.id 
              ...
              <CardContent className="p-4">
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-gray-600 mt-1 ml-8">{module.description}</p>
                    <div className="flex items-baseline gap-1 mt-2 ml-8">
                      <span className="font-bold text-[#1e3a5f]">¥{module.price_min?.toLocaleString()}</span>
                      <span className="text-sm text-gray-500">{module.price_unit}</span>
                    </div>
                  </div>
                </div>
