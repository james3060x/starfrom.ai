...
export async function GET(
  ...
  try {
    ...
    if (error || !session) {
      await logApiCall(null, auth.workspaceId!, `/v1/sessions/${params.session_id}`, 'GET', 404);
      return NextResponse.json(
    ...
    await logApiCall(null, auth.workspaceId!, `/v1/sessions/${params.session_id}`, 'GET', 200, latency);
    ...
    const latency = Date.now() - startTime;
    await logApiCall(null, auth.workspaceId!, `/v1/sessions/${params.session_id}`, 'GET', 500, latency, undefined, String(error));
...
export async function DELETE(
  ...
  try {
    ...
    if (error) {
      await logApiCall(null, auth.workspaceId!, `/v1/sessions/${params.session_id}`, 'DELETE', 500);
      return NextResponse.json(
    ...
    const latency = Date.now() - startTime;
    await logApiCall(null, auth.workspaceId!, `/v1/sessions/${params.session_id}`, 'DELETE', 200, latency);
    ...
    const latency = Date.now() - startTime;
    await logApiCall(null, auth.workspaceId!, `/v1/sessions/${params.session_id}`, 'DELETE', 500, latency, undefined, String(error));
